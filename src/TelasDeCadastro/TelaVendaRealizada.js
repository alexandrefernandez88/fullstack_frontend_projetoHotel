...
export default function TelaVendaRealizada(props) {

    const localRecursos = "http://localhost:4000/vendaproduto";
    ...
    if (exibirTabela) {
        if (status == STATUS.ocioso) {
            ...
            return (
                <Pagina>
                    <TabelaDeVenda dados={listaVendas} onTabela={setExibirTabela} buscarVendas={buscarVendas}/>
                </Pagina>
            );
        }
        else {
        ...
        return (
            <Pagina>
                <FormCadVenda onTabela={setExibirTabela} modoEdicao={autalizando} listaVendas={listaVendas} buscarVendas={buscarVendas} />
            </Pagina>
        );
    }
}